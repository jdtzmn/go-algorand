...
const MissingSignatureError = errorString("missing signature")
...
func stxnVerifyCore(s *transactions.SignedTxn, txnIdx int, groupCtx *GroupContext, batchVerifier *crypto.BatchVerifier) error {
	...
	if numSigs > 1 {
		return fmt.Errorf("signedtxn should only have one of Sig or Msig or LogicSig")
	}
	...
	if hasMsig {
		...
		return fmt.Errorf("multisig validation failed")
	}
	if hasLogicSig {
		return logicSigBatchVerify(s, txnIdx, groupCtx, batchVerifier)
	}
	return fmt.Errorf("has one mystery sig. WAT?")
}
...
func LogicSigSanityCheckBatchVerify(txn *transactions.SignedTxn, groupIndex int, groupCtx *GroupContext, batchVerifier *crypto.BatchVerifier) error {
	...
	if groupCtx.consensusParams.LogicSigVersion == 0 {
		return fmt.Errorf("LogicSig not enabled")
	}
	if len(lsig.Logic) == 0 {
		return fmt.Errorf("LogicSig.Logic empty")
	}
	version, vlen := binary.Uvarint(lsig.Logic)
	if vlen <= 0 {
		return fmt.Errorf("LogicSig.Logic bad version")
	}
	if version > groupCtx.consensusParams.LogicSigVersion {
		return fmt.Errorf("LogicSig.Logic version too new")
	}
	if uint64(lsig.Len()) > groupCtx.consensusParams.LogicSigMaxSize {
		return fmt.Errorf("LogicSig.Logic too long")
	}

	if groupIndex < 0 {
		return fmt.Errorf("Negative groupIndex")
	}
	...
	if numSigs > 1 {
		return fmt.Errorf("LogicSig should only have one of Sig or Msig but has more than one")
	}
	...
	if !hasMsig {
		...
		if ok, _ := crypto.MultisigBatchVerify(&program, crypto.Digest(txn.Authorizer()), lsig.Msig, batchVerifier); !ok {
			return fmt.Errorf("logic multisig validation failed")
		}
	}
	return nil
}
